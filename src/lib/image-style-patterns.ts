...
function colorInstruction(brandColor?: string): string {
  if (brandColor) {
    return `The dominant body color must closely match the brand color ${brandColor}. Add 1-2 additional harmonious accent colors (a brighter/lighter shade of ${brandColor}, or a soft off-white) for secondary or functional parts — do not render the entire object in ${brandColor} alone, different parts must still be distinguishable by color. Colors must be highly saturated and vivid, never muted, dusty, or pastel. Never use black, near-black, or charcoal/graphite tones anywhere on the object. Keep transitions smooth and avoid harsh clashing.`;
  }
