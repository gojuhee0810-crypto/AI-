...
`src/lib/style2-design-system.ts`의 디자인 시스템을 참조한다.
...
1. `docs/patterns/image-style-patterns.md`와 `src/lib/style2-design-system.ts`를 읽고 두 스타일의 구조를 파악한다.
...
5. 조사 결과를 `src/lib/style2-design-system.ts`의 `SemanticBlueprint` 형식으로 **초안**을 작성한다 — Must Have(없으면 인식 불가한 것만, 2~3개 이하), Should Have(있으면 좋은 것, 1~2개), Avoid(이 오브젝트에서 특히 헷갈리기 쉬운 디테일 — 예: 통장이면 "spiral binding, notebook rings, passport stamp texture"), Recognition Cue(단 하나의 핵심 특징).
6. 오브젝트가 속하는 카테고리(finance/payment/reward/travel/insurance/map/medical/commerce/coupon 중 하나)를 판단한다 — `COLOR_TOKENS`에서 컬러를 결정하는 데 쓰인다.
