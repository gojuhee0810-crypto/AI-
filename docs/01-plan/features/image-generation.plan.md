...
| FR-02 | 입력한 오브젝트를 블루프린트(Must Have/Should Have/Avoid/Recognition Cue)로 보강한 뒤 **두 스타일 모두의** 프롬프트에 넣는다 | High | **Done** |
...
**FR-02는 2026-08-13 오후까지 절반만 맞았다.** 오전에 이 표를 갱신하면서 `Done`으로
적었는데, 블루프린트를 받는 건 **스타일 2뿐이었다.** 스타일 1은 오브젝트 이름만
템플릿에 꽂고 있었다 — 실제 API를 처음 돌려보고 알았다. "전기자전거"를 넣었더니
배터리도 모터도 없는 평범한 자전거가 나왔는데, 블루프린트에는
`RECOGNITION CUE: Visible battery pack integrated into the frame's downtube`가
정확히 적혀 있었다. **만들어놓고 한쪽에만 주고 있었다.**

지금은 `objectDetailForStyle1()`이 블루프린트에서 모양에 관한 넷(CONSTRUCTION ·
SILHOUETTE · RECOGNITION CUE · AVOID)만 뽑아 스타일 1 프롬프트에 넣는다. 색·비율·
카메라는 가져오지 않는다 — 스타일 1은 자기 팔레트와 카메라 규칙이 있고, 블루프린트의
CATEGORY 색은 스타일 2(2D)용이라 섞으면 3D 아이콘이 2D 팔레트로 칠해진다.
프롬프트가 3,351자 → 3,911자가 됐고 `battery`가 들어갔다(실측).

두 스타일이 이제 같은 블루프린트를 동시에 요청하므로 `resolveObjectBlueprint`에
in-flight 잠금을 뒀다. 없으면 캐시가 빈 오브젝트에서 Claude를 두 번 부르고, 두
스타일이 서로 다른 블루프린트를 받아 한 화면에 다른 사물이 나올 수 있다.
...
9. [~] 생성 모듈 테스트 — `banner-image` 11개 · `banner-image-client` 8개 · `prompt-compiler` 20개 추가(2026-08-13). `style1/style2-generate`의 외부 API 호출부는 여전히 미검증
10. [x] **실제 API 경로 검증** (2026-08-13) — 라이브러리에 없는 오브젝트로 전 경로를 태웠다.
    이미지 78초·2장·240×240 PNG·투명 배경, 카피 10.5초·3안·글자수 전부 통과.
    카피 검증 루프가 실제로 발동해 오브젝트명 재노출을 잡고 보정했다(서버 로그 확인).
    문서의 "카피 20~40초"는 실측 10.5초로 맞지 않는다 — 느린 쪽은 이미지다.
11. [ ] **이미지 대기시간** — 실측 68~78초. 문서의 "수십 초"보다 길다
12. [ ] **생성 결과 품질 확인 방법이 없다** — 블루프린트를 넣어 프롬프트는 좋아졌지만,
    나온 그림이 그 지시를 따랐는지는 여전히 사람이 눈으로 볼 수밖에 없다
